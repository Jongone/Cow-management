...
public class Cowmanager {  // 2. 메뉴의 번호항목의 내용을 입력하는 공간임 
	...
	public void addCow() {
		...
		Cow cow =new Cow(); //위에 Cow cow; 가 삭제되서 위에거 바꾸거 인스턴스로 다시 넣어준거 (간단한 내용)
		System.out.println("Cow ID:");
		cow.id = sc.nextInt();
		System.out.println("Cow Birth(출생년도):");
		cow.birth = sc.nextInt();
		System.out.println("Cow Sex(암/수):");
		String cs= sc.next(); //이 부분 사용자의 입력을 받아 그입력값을 다른 클래스에 집어넣는 과정을 잘 만든거임 한번 보자! 내가 만든 프로그램 ... 이거는 차량 시물레이션 3주차 과제에도 나온다
